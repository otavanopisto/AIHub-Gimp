	...
	class ImageDialog(GimpUi.Dialog):
		...
		def on_workflow_selected(self, combo):
			...
			if (has_advanced):
				...
				run_button = Gtk.Button(label="Run Workflow")
				run_button.connect("clicked", self.on_run_workflow)
				#align the run button to the right
				run_button.set_halign(Gtk.Align.END)
				run_button.set_hexpand(False)

				self.workflow_elements.pack_start(run_button, False, False, 0)
